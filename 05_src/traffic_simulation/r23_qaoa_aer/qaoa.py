...
def _statevector_and_expectation(input_data: R23Input, gamma, beta, *, seed: int, optimization_level: int) -> tuple[float, dict[str, float], dict[str, float]]:
    ...
    return expectation, probabilities, {"untranspiled_depth": circuit.depth(), "transpiled_depth": transpiled.depth(), "gate_count": sum(transpiled.count_ops().values()), "two_qubit_gate_count": sum(v for k, v in transpiled.count_ops().items() if k in {"cx", "ecr", "cz"}), "parameter_count": 2 * len(gamma), "logical_qubits": input_data.n_logical}
...
def run_single(input_data: R23Input, config: R23Config) -> dict[str, Any]:
    ...
    def objective(values):
        ...
        value, _, _ = _statevector_and_expectation(input_data, gamma, beta, seed=config.seed, optimization_level=config.optimization_level)
    ...
    try:
        final_expectation, probabilities, circuit_metrics = _statevector_and_expectation(input_data, gamma, beta, seed=config.seed, optimization_level=config.optimization_level)
        ...
        final_expectation, probabilities, metrics, circuit_metrics = None, {}, {}, {}
