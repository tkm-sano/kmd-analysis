...
> **対象:** 版16の正式用道路属性解決
> **基準日:** 2026年7月16日
> **実装可否:** 追加決定事項が残るため、本文書だけを根拠とした実装開始は不可
...
本文書の内容は、研究責任者による採用、機械可読な決定表、Schema、独立正解および
固定試験が揃うまで正式仕様とはしない。既存の停止記録を本文書だけで採用値へ
変更してはならない。
...
一方、`OPEN-PROP-001`から`015`が残るため、現時点では次の状態とする。

```yaml
proposal_status: requires_additional_decisions
normative_specification: false
implementation_allowed: false
v16_full_rerun_allowed: false
```

追加決定、決定表、Schema、独立正解を承認した後に、正式仕様へ移す。
