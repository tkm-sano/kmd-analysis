...
The research owner waived independent human review on 2026-07-30. The waiver
does not set `acceptance_allowed=true` and MUST NOT be reported as independent
acceptance. It removes that review from the implementation sequence; pinned
hash checks, semantic validation and oracle independence from production code
remain mandatory.
...
independent human review was waived by the research owner, and pinned
Classifier-Resolver execution remains pending.
...
2. Implement the Classifier with responsibility limited to
   `criticality_level`, `selected_rule_id` and `matched_rule_ids`.
3. Implement the Resolver separately, or as an explicitly separated stage in
   one executable, to determine values, evidence selection, conflicts, review
   state and stopping outcomes.
4. Execute the Classifier and Resolver against positive, negative, boundary,
...
5. Accept a next-version relation closure containing the three known
...
6. Generate separate structural and formal artifacts for the accepted
