## Current research authority（2026-09-15）

研究設計全体の正本は[Complete Current Research Design](CURRENT_RESEARCH_DESIGN.md)、意思決定履歴は[Research Progress and Decision Record](RESEARCH_PROGRESS_AND_DECISION_RECORD.md)である。R23は[R23 current status](R23_STATUS.md)のとおり`R23_CLOSED_WITH_DOCUMENTED_LIMITATIONS`。R24はGates A-Dとrouting compatibilityがlimitations付きでacceptedとなり、final `C_eligible`をmaterialize済みである。

`NEXT_EXECUTABLE_TASK = freeze R24 instance generation specification`

詳細仕様は[仕様index](specifications/README.md)、[Routing Baseline canonical](specifications/ROUTING_BASELINE_CANONICAL.md)、[Reduced Problem canonical](specifications/R23_REDUCED_PROBLEM_CANONICAL.md)を参照する。下記のreduced-pipeline説明にはR23当時のscopeと実装履歴を残すが、current statusとNEXTは上記authorityを優先する。
...
| R23 reduced QAOA/Aer | `CLOSED_WITH_DOCUMENTED_LIMITATIONS` | [R23 current status](R23_STATUS.md); Formal A/B1/B2 and n=5 closure evidence retained within the Reduced Problem scope |
