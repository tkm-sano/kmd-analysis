...
文書ID: `DOC-SIMULATION-MODEL-DEVELOPMENT-VV`
...
> **現在地**: v17属性解決段階1〜12は合格。段階12は2回のrunの単体検査、主要5成果物一致、実行条件比較、`finalize()`、原子的公開まで完了。108,189件の統制対象blockerが残るため`formal_build_ready`は`false`。段階13は未着手、段階14はゲート待ち。交通モデルの較正、独立妥当性確認、正式配送比較、熟練運転者の感度分析は未実施。
...
#### 0.6.2 段階13でFormal blockerを根本原因別に解消する
...
| 実装検証（Verification） | 実装・構造の検証 | 決めた式、規則、データ変換、道路構造が正しく実装されているかの確認 |
| 妥当性確認（Validation） | 利用目的に対する妥当性確認 | モデル出力を未使用の観測や独立した根拠と比較し、定めた用途に使用できるかを判断すること |
