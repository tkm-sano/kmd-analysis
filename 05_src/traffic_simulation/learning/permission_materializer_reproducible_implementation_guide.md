...
| `build_sumo_network.py prepare` | 未実装 | 文書上の予定のみ |
...
次版関係データ参照補完、重要度分類処理、道路属性解決処理、仮道路構造の対応履歴生成を
実装する必要がある。一方、小型の通行権限反映用試験データと本体の開発は、
実データ前提工程と並行して進められる。
...
`build_sumo_network.py prepare`は現時点で存在しない。実装時は少なくとも次を
一つの処理単位として行う。
...
識別、候補母集団の再集計、新しい設定識別子・パス・SHA-256が揃うまで、実データの
判定事実生成処理を実行しない。
...
- [ ] 次版設定識別子がv15と分離されている。
- [ ] `restriction:bus` 3件を含む関係データ参照補完が受理済みである。
