...
| **進行中** | **SUMO道路網生成・構造検証** |
| 次 | 観測拡充、交通需要、較正、独立検証、共通配送問題、古典・QAOA、走行比較 |
...
`sumo_network.yml`を最初に作り、少なくとも次を明示する。これらを実行時の追加引数やPythonコードの暗黙値で変更しない。
...
- 車線数や速度が欠損する場合に使用するSUMO既定値または研究上の補完値
- OSM信号位置の変換規則
- ジャンクション統合、形状単純化、内部リンク、ランプ等の変換規則
...
5. `build_sumo_network.py`を実装し、設定から`netconvert`コマンドを機械生成する。
6. 実PBFや外部通信を必要としない`test_sumo_network.py`を作り、コマンド生成、入力不整合、出力検証、失敗時清掃を検査する。
7. Docker内で単体テストを通した後、登録済み大田区PBFを使って正式変換を1回実行する。
8. 生成した`net.xml`をSUMO CLIで無需要状態として読み込み、終了コードと警告を保存する。
9. ノード、エッジ、レーン、ジャンクション、接続、信号、内部エッジ、孤立成分を機械集計する。
10. 高速道路、主要幹線、行政界端、橋梁・トンネル、羽田空港・臨海部等の主要接続を構造検査する。
11. `render_sumo_network.py`でOSM道路とSUMO道路網を重ね、欠落、切断、過剰統合、信号位置をレビューする。
12. 入力SHA-256、設定SHA-256、実行コマンド、版、件数、警告、失敗、修正、恣意性を生成記録Markdownへ残す。
13. 生データ、`net.xml`、品質サマリー、HTMLがGit除外対象であることを確認する。
14. 全完了条件を満たした後だけ、`research_stage.yml`の`sumo_network`を`completed`へ変更し、次工程を`in_progress`へ進める。

正式実行コマンドは次の形に統一する。

```bash
docker compose run --rm analysis \
  python -m traffic_simulation.network.build_sumo_network \
  --region ota_ward \
