...
- 4. JARTIC・OSM基礎入力
- 5. 入力道路・観測点レビュー地図
- **6. SUMO道路網生成・構造検証（進行中）**
- 7. 観測拡充・一般交通・配送EV需要
- 8. 最適化基盤検証：配送制約からEV制約まで段階追加
- 9. 信号・車両・運転行動設定
- 10. 交通モデル較正
- 11. 独立データ検証
- 12. 天候・事故等の環境シナリオ固定
- 13. 正式コスト行列・共通配送問題固定
- 14. 古典最適化・Aer QAOA正式比較
- 15. 同一SUMO環境で道路経路化・走行比較
- 16. 運転挙動異質性の感度分析
- 17. EV配送シナリオ・配送需要充足人口相当評価
- 18. 段階的な空間拡張
- 19. CI・再現性検査
- 20. hayate再現
- 21. 最終成果物・限界の固定
...
> 東京の公開実測値で較正・独立検証した交通近似モデル上で、同一の合成EV配送問題を未最適化基準、古典最適化、Qiskit Aer上のQAOAで解き、大田区内の配送需要充足人口相当とその変化を比較する。

#### 9.1.1 最優先の研究質問と成果指標

最優先の研究質問は次とする。

> 同一の車両、積載量、電池、出発時刻、交通、天候、需要および計算条件の下で、配送順序の最適化により、大田区内の配送需要充足人口相当は未最適化基準から何人相当変化するか。また、古典最適化とAer QAOAでその値はどのように異なるか。

主要成果指標は、施設数または配送先数ではなく、次に限定する。

- 未最適化基準の配送需要充足人口相当 `P_baseline`
- 古典最適化の配送需要充足人口相当 `P_classical`
- Aer QAOAの配送需要充足人口相当 `P_qaoa`
...
#### 9.18.1 配送需要充足人口相当の定義

人口メッシュ`g`の公開人口を`N_g`、事前固定した1人・評価期間当たりの配送需要を`q`、解法`m`で期限内に配送を完了した量を`C_gm`とする。メッシュ別配送需要充足人口相当は次とする。
...
実顧客や個人の受取記録ではないため、`P_m`は「配送需要充足人口相当」と表記する。「配送できた実人数」「サービスを受けた住民数」「将来配送可能な人口」とは表記しない。
...
配送需要充足人口相当を主要指標とし、配送完了率、走行距離、旅行時間、遅延、充電回数、充電待ち、電力消費、電池切れ、急減速、停止回数、一般交通への影響を説明用の副次指標とする。平均値だけでなく、分布、分散、信頼区間、下位分位点、失敗試行を保存する。
...
最終報告には、配送需要充足人口相当を最優先アウトカムとして示し、人口原典、メッシュ版、需要換算`q`、評価期間、未最適化基準、古典・QAOA差、信頼区間を併記する。また、対象地域と期間、使用観測データ、推定・仮定した項目、較正と独立検証の分離、誤差、不確実性、乱数シード間変動、大田区から東京全域への一般化限界、海外運転挙動差の移転仮定、Qiskit Aerがシミュレーターであること、QAOAの問題規模と資源、量子優位性を実証していないこと、完全な実環境再現でも実受取人数でもないことを明記する。
...
- [ ] 配送需要充足人口相当について、人口原典、メッシュ版、基準日、換算式、上限制約、入力・設定SHA-256を追跡できる。
- [ ] 配送需要充足人口相当を実際に配送を受けた人数と表現していない。
