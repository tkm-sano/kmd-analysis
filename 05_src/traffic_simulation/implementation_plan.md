...
flowchart TD
    ...
    S16[16. 熟練ドライバー差の感度分析]
    S17[17. EV配送シナリオ・需要充足人口相当評価]
    S18[18. 段階的な空間拡張]
    S19[19. CI・再現性検査]
    S20[20. hayate再現]
    S21[21. 最終成果物・限界の固定]
...
本計画の最終目的は、東京の公開実測データで道路交通を較正・検証し、その同一交通環境と同一配送問題に対して、古典最適化とQiskit Aer上のQAOAで配送ルートを生成し、EV配送と熟練運転者差を含む走行結果を比較できるモデルを構築することである。その上で最も重視する最終アウトカムは、大田区内で未最適化基準、古典最適化、Aer QAOAを用いた場合に、設定期間内に充足できた合成配送需要を人口換算した値がどれだけ変化するかである。公開データだけでは、全車両の実OD、実配送軌跡、全交差点の信号現示、全道路の速度、実顧客別配送需要を取得できない。そのため、最終成果を「東京の完全なデジタルツイン」「実交通の完全再現」または「実際に配送を受けた人数」とは位置付けない。

最終的な表現は次に統一する。

> 東京の公開実測値で較正・独立検証した交通近似モデル上で、同一の合成EV配送問題を未最適化基準、古典最適化、Qiskit Aer上のQAOAで解き、大田区内の需要充足人口相当とその変化を比較する。

#### 9.1.1 最優先の研究質問と成果指標

最優先の研究質問は次とする。

> 同一の車両、積載量、電池、出発時刻、交通、天候、需要および計算条件の下で、配送順序の最適化により、大田区内の対象期間における需要充足人口相当は未最適化基準から何人相当変化するか。また、古典最適化とAer QAOAでその値はどのように異なるか。

主要成果指標は、施設数または配送先数ではなく、次に限定する。

- 未最適化基準の需要充足人口相当 `P_eq_baseline`
- 古典最適化の需要充足人口相当 `P_eq_classical`
- Aer QAOAの需要充足人口相当 `P_eq_qaoa`
- `P_eq_classical - P_eq_baseline`
- `P_eq_qaoa - P_eq_baseline`
- `P_eq_qaoa - P_eq_classical`
- 大田区対象人口に対する需要充足人口率
- 未充足需要人口相当
...
最終的に最も示したい図は、縦軸に需要充足人口相当、横軸に量子計算資源を置き、同一問題に対する古典最適化の需要充足人口相当を水平基準線として、QAOAの各設定を点で示す資源―社会的効果図である。横軸はlogical qubit数を有力候補とするが、正式実験前の時点では未確定とし、結果を見て都合のよい指標を選ばない。
...
- SUMOと`netconvert`の実行環境は、digest固定した`sumo`サービスだけを正本とする。
...
- `.netccfg`は`sumo_network.yml`から生成する実行時成果物であり、設定の正本としてGit管理しない。
...
05_src/quantum_optimization/resource_analysis.py
...
#### 9.11.10 QUBO構造から論理リソースを評価する分岐

QUBO生成後は、QAOAを実行して配送ルートを得る分岐とは別に、QUBOそのものから論理問題規模とリソース要件を集計する。これはSUMO走行結果やQAOA測定結果を入力とせず、同一のQUBO成果物から再計算可能にする。

QUBO構造から少なくとも次を保存する。

- 元問題の変数数と制約種別別件数
- QUBO変換後の二値変数数
- 補助変数、slack変数、符号化変数の件数
- Q行列の次元
- 非ゼロ線形項数と非ゼロ二次項数
- QUBO密度
- 目的項と制約ペナルティ項の件数
- 非ゼロ係数の最小絶対値、最大絶対値、符号別件数、dynamic range
- QUBO相互作用グラフのノード数、エッジ数、平均次数、最大次数、連結成分数
- 直接符号化時に必要なlogical qubit数
- Ising変換後の定数項、Z項数、ZZ項数
- QAOA cost layer 1層当たりの理論上の1量子ビット・2量子ビット相互作用数
- `reps`を掛けたtranspile前の論理回路規模推定
- 極小問題で列挙可能な場合の全ビット列数、実行可能ビット列数、実行可能状態割合

次の3種類を混同しない。

| 区分 | 算出時点 | 内容 |
|---|---|---|
| QUBO論理リソース | QUBO・Ising生成直後 | 二値変数、logical qubit、項数、密度、係数、相互作用グラフ |
| QAOA回路リソース | QAOA回路生成・transpile後 | 回路深さ、ゲート数、2量子ビットゲート数、transpiler設定 |
| Aer実行リソース | QAOA実行後 | shots、評価回数、反復数、時間、CPU/GPU、メモリ、成功確率 |

Aerは物理量子ハードウェアではないため、QUBOの二値変数数を実機の必要physical qubit数と表現しない。ハードウェア接続制約、routing、誤り訂正を含むphysical qubit推定は本基準実験の対象外とし、行う場合は別手法・別前提の資源推定として分離する。

各`constraint_stage_id`、顧客数、車両数、時間枠数、充電候補数ごとにQUBO論理リソースを集計し、制約を一段追加したときの増分を保存する。成果物は少なくとも`qubo_resource_summary.json`、`qaoa_circuit_summary.json`、`aer_execution_summary.json`へ分離し、それぞれに問題、QUBO、設定のSHA-256を記録する。
...
古典最適化とQiskit Aer QAOAには、同じ道路網、需要、天候、事故・規制、信号、車両、出発時刻、地点間コスト、乱数シード群を与える。解法間で異なる環境情報を使用しない。動的再最適化を比較する場合も、更新時刻、観測可能な情報、再計算時間予算を両方式で揃える。
...
3. 同じ凍結済みインスタンスを古典ソルバーへ渡すとともにQUBO・Isingへ変換する。
4. QUBO構造から論理変数、logical qubit、項数、密度、係数範囲、相互作用グラフを集計する。
5. 同じQUBOをQiskit Aer QAOAへ渡し、回路・実行リソースとraw解を保存する。
6. 古典解とQAOA解について、復号結果、制約状態、目的値、計算資源、実行時間を比較可能なスキーマで保存する。
7. 両方の訪問順序を同じ道路経路生成規則でSUMOエッジ列へ変換する。
8. 同じ出発時刻、一般交通、信号、車両設定、環境シナリオ、SUMO seed群で走行評価する。
9. 需要充足人口相当、解品質、SUMO走行結果、QUBO論理リソース、QAOA回路・実行リソースを分けて集計した後に統合比較する。
...
### 9.18 段階15：EV配送シナリオと需要充足人口相当を評価する
...
公開統計から共通需要を作る処理と、未最適化基準の正式仕様は`05_src/traffic_simulation/demand/baseline_demand_and_comparator.md`を正本とする。本節と正本が矛盾する場合は計算を停止する。

人口メッシュ`m`の公開人口を`N_m`、同メッシュについて事前固定した1人・1日当たりの合成配送需要量を`q_m`、対象日数を`T`、解法`s`で対象期間内かつ期限内に配送を完了した宅配便個数相当量を`Y_ms`とする。需要充足人口相当は次とする。

```text
P_eq(s) = sum_m min(N_m, Y_ms / (q_m * T))
```

`q_m`、`T`、`Y_ms`の単位はそれぞれ「宅配便個数相当／人・日」「日」「宅配便個数相当」とする。これにより、分母`q_m * T`は1人・評価期間当たりの需要となる。`q_m`は物資または荷物の種類、単位、評価期間と共に実験前に固定し、結果に合わせて変更しない。初期比較では全メッシュ共通の`q_base`を用いるため`q_m = q_base`とする。複数品目を扱う場合は品目別に需要充足人口相当を計算し、換算根拠の異なる品目を無根拠に合算しない。

計算条件として`N_m >= 0`、`q_m > 0`、`T > 0`、`Y_ms >= 0`を必須とする。`q_m`が欠損または0以下のメッシュを0人として黙って処理せず、集計を停止して品質エラーにする。

本モデルでは、国土交通省が公表した令和6年度の全国宅配便取扱個数5,031,470,000個を、同年度の代表人口として採用した2024年10月1日現在の総人口123,802,000人および365日で除し、**全国宅配便取扱個数の人口換算原単位**を算出する。算出値は0.111345934個／人・日である。これは、住民個人の注文率、個人宅向けの1人当たり配送需要、住宅向け配送の実測値または生活上の必要量ではなく、全国の宅配便取扱実績を総人口で基準化した派生的な人口原単位である。

各人口メッシュには、当該原単位と居住人口との積を宅配便個数相当量として配分する。ただし、この配分は全国の宅配便が居住人口に比例して発生すると仮定した基準シナリオであり、事業所向け配送、地域別EC利用率、昼間人口、商業・産業集積、荷物の同時配達および再配達を反映していない。したがって、算出値を配送先数、顧客数または配送停止回数として直接解釈しない。

```text
q_base = 5,031,470,000 / 123,802,000 / 365
       = 0.111345933952 個/人・日
報告時表示 = 0.111 個/人・日
```
...
- 地域配分：人口比例
- メール便：対象外
- 来歴区分：`derived_national_proxy`（モデル区分は推定値）
- 宅配便原典：国土交通省「令和6年度 宅配便・メール便取扱実績について」<https://www.mlit.go.jp/report/press/jidosha04_hh_000341.html>
- 人口原典：総務省統計局「人口推計（2024年10月1日現在）」<https://www.stat.go.jp/data/jinsui/2024np/index.html>

計算内部では丸め前の`q_base`を使用し、`0.111`は表示値に限る。大田区の総需要個数は`q_base`と対象人口の積を四捨五入する。各人口メッシュにはまず需要期待値の整数部分を割り当て、残りを小数部分の大きい順に1個ずつ配る最大剰余法を使用する。同率時の順序は人口メッシュIDの昇順に固定し、乱数で変えない。

大田区固有または住宅向けに限定した根拠データへ将来置き換える場合は、別の需要設定版と実験IDを作り、初期基準結果を上書きしない。「住民の実注文率」「1人当たりの生活上の必要量」とも表現しない。

2026年7月18日現在、合成需要までの実装とDocker内実行は完了している。正本は次とする。
...
- 計算仕様：`05_src/traffic_simulation/demand/baseline_demand_and_comparator.md`
- 取得・実行記録：`03_data/metadata/acquisition/20260718_ota_baseline_open_statistics_acquisition.md`

実行結果は、N03大田区境界と交差する500mメッシュ191件、全面包含122件、境界メッシュ69件、2024年配分人口736,652人、1日分の整数合成需要82,023宅配便個数相当である。面積按分した2020年人口は747,271.088683人相当、中心点包含法では736,737人で、境界規則による差は10,534.088683人相当であった。この差を真値誤差とは解釈しない。

生成物は`03_data/processed/traffic_simulation/`以下へ保存しGit管理しない。出力ハッシュ、設定ハッシュ、原本ハッシュ、交差件数、人口・需要合計は品質要約へ保存する。集約配送点、配送停止回数、未最適化ルート、SUMO走行、需要充足人口相当はこの実行範囲に含めない。

東京都市圏物資流動調査の「回／世帯・週」は宅配の受取頻度であり、全国宅配便取扱実績の「個／人・日」とは、単位、母集団、集計対象および計数対象が異なる。両者を直接除算して「東京補正係数」を作成せず、`q_base`の地域補正にも直接使用しない。両統計は次の独立した系列として扱う。

- 全国宅配便取扱実績：宅配便個数相当量による基準シナリオの原単位
- 東京都市圏物資流動調査：世帯当たり受取頻度、再配達割合、受取方法等の外部記述統計および整合性確認

将来、受取1回当たりの箱数、対象世帯率、世帯人数、個人宅向け割合など、両単位を接続する根拠が揃った場合に限り、別の変換モデルとして実装する。その場合も変換式、対象範囲、基準日、不確実性、出典、設定版を保存し、全国原単位による基準シナリオとは別に報告する。

人口メッシュは重複しない母集団として使用する。配送地点は計算上の代表点であり、地点周辺人口を重複加算しない。メッシュ内に複数の内部配送地点を置く場合も、メッシュ`m`の寄与`min(N_m, Y_ms / (q_m * T))`は`N_m`を上限とする。

`Y_ms`へ含める配送は、少なくとも次をすべて満たすものに限定する。
...
実顧客や個人の受取記録ではないため、主要名称は「需要充足人口相当」とし、記号は`P_eq`に統一する。「配送できた実人数」「サービスを受けた住民数」または「個人宅へ配送できた人数」とは表記しない。「需要等価人口」は補足的な言い換えに限り、表・図・設定・成果物の正式名称には使用しない。

#### 9.18.2 比較と副次指標

未最適化基準、古典最適化、Aer QAOAについて同じ人口メッシュ、`q_m`、対象日数`T`、需要総量、車両、交通・環境シナリオ、seed群を使用し、`P_eq_baseline`、`P_eq_classical`、`P_eq_qaoa`と各差分を計算する。未最適化基準の配送規則も結果を見る前に固定する。

需要充足人口相当を主要指標とし、配送完了率、走行距離、旅行時間、遅延、充電回数、充電待ち、電力消費、電池切れ、急減速、停止回数、一般交通への影響を説明用の副次指標とする。平均値だけでなく、分布、分散、信頼区間、下位分位点、失敗試行を保存する。

完了条件は、人口原典、メッシュ版、人口基準日、需要換算`q_m`、対象日数`T`、配送完了量`Y_ms`の判定、3方式の入力SHA-256、SUMO seed群から、需要充足人口相当の集計値と差分を第三者が再計算できることである。

#### 9.18.3 最終的に示す資源―社会的効果図

研究の最終的な主要図として、次の関係を可視化する。
...
この図の意味は、同一の凍結済み配送問題について、古典最適化が達成した需要充足人口相当を基準とし、各QAOA設定がどの程度の量子計算資源で、どの程度の需要充足人口相当を達成したかを示すことである。QAOAの各点には、少なくとも`constraint_stage_id`、問題規模、seed群、QUBO SHA-256、QAOA設定SHA-256を対応付ける。

横軸の第一候補はlogical qubit数とする。ただし、logical qubit数だけでは同一QUBOに対するshots、`reps`、古典オプティマイザー、回路評価回数等の違いを表せないため、最終横軸は未確定とする。候補は次とする。

- logical qubit数
- QUBO非ゼロ二次項数または相互作用グラフのエッジ数
- transpile後の2量子ビットゲート数
- 回路深さ
- 回路評価回数
- Aer総実行時間
- 最大メモリ使用量

正式比較の実行前に、主横軸を1つ、補助パネルの横軸を必要数だけ設定ファイルへ固定する。各資源は単位が異なるため、恣意的な重みで単一の複合資源スコアへまとめない。logical qubit数を用いる場合も、実機physical qubit数とは表現しない。

古典最適化の水平線には、同じ問題、交通・環境条件、人口換算、seed群から得た`P_eq_classical`を使用する。必要に応じて未最適化基準`P_eq_baseline`も別の水平線として表示する。QAOA点にはseed間の需要充足人口相当の信頼区間または分位範囲を縦方向の誤差線として付け、実行可能解を得られなかった設定を図から除外せず識別可能に表示する。

この図は「量子ビットが人口を直接増加させる」という因果関係を示すものではない。計算資源を用いて得たQAOA解を同一SUMO環境で評価したときの社会的効果との対応関係を示す。Aer上の関係であり、量子優位性または実量子ハードウェア上の性能を示すものではない。

出力候補は次とする。

```text
06_outputs/traffic_simulation/figures/resource_social_effect_frontier.png
06_outputs/traffic_simulation/figures/resource_social_effect_frontier.pdf
06_outputs/traffic_simulation/tables/resource_social_effect_points.csv
```
...
最終報告には、需要充足人口相当を最優先アウトカムとして示し、人口原典、メッシュ版、需要換算`q_m`、対象日数`T`、配送完了量`Y_ms`の判定、未最適化基準、古典・QAOA差、信頼区間を併記する。また、対象地域と期間、使用観測データ、推定・仮定した項目、較正と独立検証の分離、誤差、不確実性、乱数シード間変動、大田区から東京全域への一般化限界、熟練係数の仮説性、Qiskit Aerがシミュレーターであること、QAOAの問題規模と資源、量子優位性を実証していないこと、完全な実環境再現でも実受取人数でもないことを明記する。
...
- [ ] 各`constraint_stage_id`について、QUBO変数、補助変数、logical qubit、非ゼロ項、密度、係数範囲、相互作用グラフと前段からの増分を再計算できる。
- [ ] QUBO論理リソース、QAOA回路リソース、Aer実行リソース、SUMO配送効果が別成果物として保存され、最後に統合比較されている。
- [ ] logical qubitと実機physical qubitを混同していない。
...
- [ ] 大田区内の`P_eq_baseline`、`P_eq_classical`、`P_eq_qaoa`と各差分が、同一の人口メッシュ、需要換算`q_m`、対象日数`T`、配送完了量`Y_ms`の判定、車両、交通・環境条件から計算されている。
- [ ] 需要充足人口相当について、人口原典、メッシュ版、基準日、換算式、上限制約、入力・設定SHA-256を追跡できる。
- [ ] 需要充足人口相当を実際に配送を受けた人数と表現していない。
- [ ] 古典最適化を水平基準線、QAOAを設定別の点とする資源―社会的効果図が生成され、主横軸が正式実験前に固定されている。
