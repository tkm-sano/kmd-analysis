...
- [6. 属性値解決と版16全件実行が未完了](#6-属性値解決と版16全件実行が未完了)
...
```mermaid
graph TD
    A[属性値解決処理が未実装] --> B[道路属性の正式全件解決ができない]
    V[版16母集団が分類・解決処理へ未接続] --> B
...
版16母集団の重要度分類・属性値解決の統合実行と阻害項目0件を意味しない。したがって、
本問題の状態は未解決のままである。
...
- 明示値、外部証拠、検証済みモデル、構造確認用代替値から採用値を決める
  属性値解決処理
- 証拠選択、競合処理、確認状態、停止コードを決める処理
- 受理済み版16母集団を判定事実生成、重要度分類、属性値解決へ接続する統合処理
- 全道路、`lanes`と`maxspeed`、`structural`と`formal`を被覆する実データ成果物
- 統合成果物に対する意味整合検査と原子的公開の実行証拠
...
実装済みの分類処理はこの責務分離を満たしている。しかし、値を採用する属性値解決
処理と版16実データへの全件適用が残るため、問題全体は未解決である。
...
重要度分類実装後の直近の交通シミュレーション検証一式は`340 passed`であり、
...
| 優先 | 作業 | 解消する問題 | 並行実施 |
|---:|---|---|---|
| 1 | 属性値解決処理を実装し、固定試験用データで検証する | `ISSUE-ATTR-001`、`ISSUE-ATTR-002` | 通行権限反映の小型試験と並行可能 |
| 2 | 307件の例外について証拠条件付きの値解決規則を実装する | `ISSUE-ATTR-003` | 属性値解決処理と一体で実施 |
| 3 | 受理済み版16母集団を全件分類・解決する | `ISSUE-ATTR-001`、`ISSUE-ATTR-002`、`ISSUE-ATTR-003` | 工程1・2完了後 |
