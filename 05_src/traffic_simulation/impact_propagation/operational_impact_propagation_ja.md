# 配送評価における技術・人間要因・運用・人口代理指標の概念体系

## 概念体系の位置づけ

本資料は、本研究で扱う複数の技術的側面、人間要因および運用的側面を、地域単位および人口単位を介して、最終的な人口代理指標である「配送需要充足人口相当」へ対応づける。接続線は、同じ測定結果を別の観点から捉える関係、測定間の関連を分析する関係、判定時に別の測定項目を参照する関係、または同じ配送結果を別の集計単位で表す関係を示す。
...
- 後続のHuman Factors条件には、source上の運転者群、個人、走行条件、生理・視線測定条件、運転行動profileおよび時刻同期規則を含める。これらは現在のformal比較入力ではない。
...
flowchart LR
    subgraph premises["共通する評価・解釈条件"]
        conditions["需要 · 地域区分 · 道路交通 · 時間 · 車両<br/>電池 · 充電 · 運転profile · 測定条件 · 集計規則"]
    end

    subgraph technical["技術的側面・人間要因"]
        planning["配送計画の実行可能性"]
        physiology["運転者の生理・注意状態<br/>後続分析"]
        behavior["運転行動特性<br/>後続感度分析"]
    ...
    physiology ---|測定間の関連を分析する| behavior
    behavior ---|操作・反応の測定が対応する| driving
    behavior ---|加減速・制動の測定が対応する| energy
...
実線は無方向であり、ラベルに記した概念上の対応だけを示す。共通条件と技術的側面・人間要因の間の不可視リンクは描画上の配置にだけ用い、因果関係を表さない。生理状態と運転行動は関連を分析する測定群であり、生理測定が運転操作を引き起こすことを図は主張しない。地域別配送需要充足度は地域間比較のための概念であり、最終値の算出経路には用いない。配送需要充足人口相当は、地域別配送完了需要量から換算した地域別配送需要充足人口相当を合計する唯一の最終人口代理指標である。
...
| 運転者の生理・注意状態 | 統制された走行中に記録する運転者の生理および注意に関する測定状態である。 | 後続Human Factors段階で、利用可能で統制されたEEG、心拍、視線等の変数を走行ID・運転者ID・時刻へ対応づけて保存する。 | 運転者、時間区間、走行 | source定義の生理・視線単位 | 運転者別、条件別、走行別、source群別 | 事前定義した要約量と関連を分析し、単一の運転能力scoreへ統合しない。 | 本概念は測定された人間状態を表し、運転行動特性は車両操作を表す。 | 熟練度、運転の良否、医学的診断、または量子生成計画が生理反応を生じさせた証拠ではない。 |
| 運転行動特性 | 統制された走行条件で観測する車両操作および周囲交通への反応の特徴である。 | 後続感度分析で、受理済みsource dataとsimulation mappingが対応する加速、制動、操舵、反応および安定性の測定を保存する。 | 運転者、車両、時間区間、走行 | source定義の操作・反応単位 | 運転者別、条件別、走行別、source群別 | source群と個人差を分け、東京における群構成比を推定しない。 | 本概念は人間による車両操作を表し、配送走行特性は実現した距離、時間および遅延を表す。 | `source_expert`・`source_novice`は東京の配送運転者classまたは生理的因果機序を示さない。 |
...
後続Human Factors分析では、運転者、走行、条件および時刻区間を評価単位とし、
受理済みsourceに実在するEEG、心拍、視線等の測定だけを使用する。測定の
sampling rate、欠損、品質flag、同期方法、利用可能な運転者数をsourceごとに
保持する。異なる生理・視線単位を一つの熟練度scoreへ合算せず、運転行動との
関連を事前定義した分析単位で評価する。

### 運転行動特性

後続感度分析では、運転者、車両、走行条件および時刻区間を評価単位とし、
加速、制動、操舵、反応および走行安定性についてsourceが実際に提供する測定を
保存する。`source_expert`と`source_novice`は原典の群名として保持し、東京の
配送運転者の経験class、構成比または能力差へ読み替えない。生理測定との関連、
運転行動と走行・エネルギー量の対応、およびsource群差を別々に報告する。
...
運転者の生理・注意状態と運転行動特性は後続のHuman Factors分析であり、現在の
formal配送手法比較の入力ではない。現在予定するExpert Driving Datasetは、
古典手法または量子系手法が生成した配送計画を提示する実験ではない。このため、
本データからsolver、計画提示、生理反応、運転行動というHCI上の関係を実証
できない。その評価には、計画内容と提示方法を統制し、運転者の受容・逸脱、
生理測定および運転行動を同時に記録する別の実験が必要である。
...
量子系手法は配送計画の生成・比較と計算資源の評価系統に属する。Human Factors
系統との接続を主張するには、生成計画の提示、運転者の理解・受容・逸脱、生理・
注意状態および運転行動を同じ統制実験内で測定する必要がある。現在の概念図は、
その直接接続を示さず、将来必要となる測定概念だけを明示する。
