# 交通シミュレーション・Routing仕様index（現行）

Updated: 2026-09-11。Routing BaselineとReduced Problemで現在読むべき文書を本indexで一意に示す。

## 現行仕様

- [Routing Baseline 正式仕様](ROUTING_BASELINE_CANONICAL.md)
- [Reduced Problem 正式仕様](R23_REDUCED_PROBLEM_CANONICAL.md)
- [R20 supporting specification](R20_QAOA_SUBPROBLEM_SPEC.md)（係数・exact validationの詳細）
- [R23 implementation contract](R23_IMPLEMENTATION_CONTRACT.md)
- [R23 roadmap](../R23_ROADMAP.md)
- [R23 status](../R23_STATUS.md)

## Authority入口とhistorical evidence review

- [R23 final authority map](../../../reproducibility/outputs/traffic_simulation/r23_repository_rebaseline/20260913_v2/final_authority_map.json)（scientific/audit/remediationの役割と現行status）

以下のreviewはhistorical。B2 artifact integrityはFAILED、旧reviewによる受入は新たに承認しない。

- [Formal A evidence review](../../../reproducibility/outputs/traffic_simulation/r23_formal_evidence_review/20260911_v1/evidence_review.json)
- [B1 Evidence Review](../../../reproducibility/outputs/traffic_simulation/r23_experiment_b1_evidence_review/20260911_v1/README.md)
- [B2 Evidence Review](../../../reproducibility/outputs/traffic_simulation/r23_experiment_b2_evidence_review/20260911_v1/README.md)
- [Reduced Problem final review](../../../reproducibility/outputs/traffic_simulation/r23_reduced_problem_final_review/20260911_v1/reduced_problem_final_review.json)

## 旧版

`R23_FORMAL_EXPERIMENT_DESIGN_V1.md`、`R23_EXPERIMENT_B_DESIGN_V1.md`、initial proposalは、実行前の計画・proposalを保存する旧版である。結果判定や次工程の単独根拠にはしない。実行前design authorityとしての役割は保持し、amendment・raw evidenceとの関係はauthority mapで追跡する。

## 履歴

Formal A execution/corrective completion、B1 v1 failure、B1 v2、B2 v1 failure、B2 v2および各reviewは履歴・immutable evidenceとして保持する。失敗attemptを削除せず、科学比較ではreviewが指定するv2のみを使用する。
