...
Frozen inputs remain in the repository’s `03_data/processed` and `02_literature` trees. All generated data, figures, audit tables, logs, manifests, and document exports are written under `reproducibility/outputs`. This directory is intentionally not versioned: it is recreated by executing the authoritative notebook. The versioned deliverables are `quantum_transport_reproducibility_audit_revised.ipynb` and the curated final PNG tables under `06_outputs/tables/`. The complete circuit-width extraction and its registered figures remain part of the authoritative notebook workflow.
